	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("adminlogin"))
		{
			...
			if(!pagerandom.equals(random)&&request.getParameter("a")!=null)
			{
				...
				else if(utype.equals("学生"))
				{
					sql1 = "select * from xueshengxinxi where xuehao='"+username+"' and mima='"+password+"' "; 
				}
				else {
					sql1 = "select * from jiaoshixinxi where jiaoshibianhao='"+username+"' and mima='"+password+"' "; 
				}

			List<HashMap> userlist1 = dao.select(sql1);
			if(userlist1.size()==1)
			{
				request.getSession(). setAttribute("username", username);    //这句容易忘记
				if (utype.equals("学生") || utype.equals("教师"))
				{
				request.getSession(). setAttribute("cx",  utype); 
		...
		if(ac.equals("uppass"))
		{
			...
			HashMap m = dao.getmaps("jiaoshibianhao",(String)request.getSession().getAttribute("username"), "jiaoshixinxi");
			...
			String sql = "update jiaoshixinxi set mima='"+userpass+"' where jiaoshibianhao='"+(String)request.getSession().getAttribute("username")+"'";
			dao.commOper(sql);
			request.setAttribute("suc", "");
			go("mod2.jsp", request, response);
			}
		}
		if(ac.equals("uppassx"))
		{
			String olduserpass = request.getParameter("ymm");
			String userpass = request.getParameter("xmm1");
			String copyuserpass = request.getParameter("xmm2");   
			HashMap m = dao.getmaps("xuehao",(String)request.getSession().getAttribute("username"), "xueshengxinxi");
			if(!(((String)m.get("mima")).equals(olduserpass)))
			{
				request.setAttribute("error", "");
				go("mod3.jsp", request, response);
			}else{
			//String id = (String)user.get("id");
			String sql = "update xueshengxinxi set mima='"+userpass+"' where xuehao='"+(String)request.getSession().getAttribute("username")+"'";
			dao.commOper(sql);
			request.setAttribute("suc", "");
			go("mod3.jsp", request, response);
			}
